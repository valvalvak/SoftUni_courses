class PhotoAlbum:
    ...
    def add_photo(self, label: str):
        try:
            if len(self.photos[self.page_i]) == 4:
                self.page_i += 1
            self.photos[self.page_i].append(label)
            return f"{label} photo added successfully on page {self.page_i + 1} slot {len(self.photos[self.page_i])}"
        except IndexError:
    ...
    def display(self):
        ...
        dash = "-"
        photo = "[]"
        string_representation += f"{dash * 11}\n"
        for page in self.photos:
            string_representation += ' '.join([photo for photos in page if photos]) + "\n"
            string_representation += f"{dash * 11}\n"
