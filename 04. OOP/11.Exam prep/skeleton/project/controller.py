...
class Controller:
    PLAYER_TYPES = [Beginner, Advanced]
    CARDS_TYPES = [MagicCard, TrapCard]
    ...
    def add_player(self, player_type: str, username: str):
        for player in Controller.PLAYER_TYPES:
            if type(player) == player_type:
                playa = player(username)
                self.player_repository.add(playa)
                return f"Successfully added player of type {player_type} with username: {username}"

    def add_card(self, card_type: str, name: str):
        for card in Controller.CARDS_TYPES:
            if type(card) == card_type:
                the_card = card(name)
                self.card_repository.add(the_card)
